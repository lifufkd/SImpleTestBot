        ...
        if db_actions.user_is_existed_local(chat_id):
            ...
            if db_actions.user_id_registered(chat_id):
                ...
                if command == 'start':
                    ...
                    bot.send_message(user_id, 'Добро пожаловать в чат-бот опроса подписчиков - граждан России!', reply_markup=buttons.reg_btns())
    ...
    def callback(call):
        ...
        if db_actions.user_is_existed_local(user_id):
            if db_actions.user_is_admin_local(user_id):
                ...
                elif command == 'get_admin_applications':
                    data = db_actions.get_admin_applications()
                    s = str()
                    for i in data:
                        if i[3] == 1:
                            status = 'Закрыта'
